...
select d.MgrId 
from Dept d
inner join [User] u on d.Id=u.DeptId
where u.Id='{0}'
";
        //get user name
        public static string SqlUserName = "select Name from User where Id='{0}'";
        //get dept manager Id
        public static string SqlDeptMgr = "select MgrId from Dept where Id='{0}'";
        //get first role member
        public static string SqlRole = "select UserId from XpUserRole where RoleId='{0}'";
...
select 
    ...
    l.Sort, l.CondStr
from XpFlowLine l
join XpFlow f on l.FlowId=f.Id
join XpFlowNode nf on l.StartNode=nf.Id
join XpFlowNode nt on l.EndNode=nt.Id
where f.Code='{0}'
...
";
            ...
            string signNote, string sourceTable, bool isTest)
        {
            ...
            var signTable = GetSignTable(isTest);
            var sql = $"select SourceId, FlowLevel, TotalLevel from {signTable} where Id='{flowSignId}' and SignStatus='0'";
