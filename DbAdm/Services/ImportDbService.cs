...
insert into Table(Id, ProjectId, Code, Name, Note, Status)
values('{0}', '{1}', '{2}', '', '{3}', 1)
...
select
    ...
    c.Nullable
from #tmpColumn c
inner join [Table] t on t.Code=c.TableCode and t.ProjectId='{0}' 
...
", projectId));
        ...
        private void SetSqlVar(bool fromTmpTable)
        {
            if (fromTmpTable)
            {
                _sqlGetTables = "select * from tmpTable";
                _sqlGetCols = "select * from tmpColumn";
            }
            else
            {
                _sqlGetTables = @"
